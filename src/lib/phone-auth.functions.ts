...
export const requestPhoneOtp = createServerFn({ method: "POST" })
  .validator((data: { phone: string; purpose?: OtpPurpose }) => {
...
export const verifyPhoneOtp = createServerFn({ method: "POST" })
  .validator(
    (data: { phone: string; code: string; metadata?: OtpMetadata; purpose?: OtpPurpose }) => {
      ...
      const metadata: OtpMetadata | undefined = data.metadata
        ? {
            ...(typeof data.metadata.name === "string"
              ? { name: data.metadata.name.trim().slice(0, 80) }
              : {}),
