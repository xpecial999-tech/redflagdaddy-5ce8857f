...
function UpgradePage() {
  ...
  useEffect(() => {
    if (returnedId && ent.data?.paidModeEnabled) finalizeMut.mutate(returnedId);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [returnedId, ent.data?.paidModeEnabled]);

  const startMut = useMutation({
    mutationFn: () => startFn(),
  });

  const price = ent.data ? formatCheckoutPrice(ent.data.priceCents, ent.data.currency) : null;
  ...
  if (ent.error) {
    return (
      <div className="max-w-md mx-auto glass-strong rounded-3xl p-8 text-center space-y-3">
        <h1 className="text-2xl font-display">Upgrade details unavailable</h1>
        <p className="text-sm text-muted-foreground">
          We couldn't load the current plan and price. No payment has been started.
        </p>
        <button
          type="button"
          onClick={() => ent.refetch()}
  ...
  if (ent.data && !ent.data.paidModeEnabled) {
    return (
      <div className="max-w-md mx-auto glass-strong rounded-3xl p-8 text-center space-y-3">
        <CheckCircle2 className="w-10 h-10 text-emerald-400 mx-auto" />
        <h1 className="text-2xl font-display">Full access is currently included</h1>
        <p className="text-sm text-muted-foreground">
          Paid mode is off, so there is nothing to purchase.
        </p>
        <button
          type="button"
      ...
      <div className="glass-strong rounded-3xl p-6 space-y-3">
        ...
        <ul className="text-sm text-muted-foreground space-y-2">
          <li className="flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-aurora-2" /> Full 100-question assessments
          </li>
          <li className="flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-aurora-2" /> Category deep-dive journeys
          </li>
          <li className="flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-aurora-2" /> Unlimited journeys
          </li>
          <li className="flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-aurora-2" /> Print or save reports and enable share
            links
          </li>
        </ul>
        <p className="text-xs text-muted-foreground flex items-center gap-1.5">
          <ShieldCheck className="w-3.5 h-3.5" /> Checkout provided by Peach Payments
        </p>
      </div>
