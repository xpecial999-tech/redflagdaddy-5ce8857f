...
async function sendResendEmail(payload: SendableEmailPayload, apiKey: string): Promise<void> {
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      html: payload.html,
      text: payload.text,
      headers: payload.message_id ? { "X-Entity-Ref-ID": payload.message_id } : undefined,
    }),
  });
  ...
  const error = new Error(
    `Resend API returned ${response.status}${body ? `: ${body.slice(0, 500)}` : ""}`,
  ) as ResendError;
  ...
  if (retryAfter) {
    ...
    error.retryAfterSeconds = Number.isFinite(parsed) ? parsed : null;
  }
